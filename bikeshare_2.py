import time
# see more at https://docs.python.org/3/library/calendar.html
import calendar
...
def get_filters():
    ...
    while city not in CITY_DATA.keys():
        print("\n Valid inputs here: \n", CITY_DATA.keys())
        city = input('Please specify city of interest: ')
    print("\n Your city of interest is ",city)

    # get user input for month (all, january, february, ... , june)
    month = 'not specified'

    while month not in MONTHS[1:] and (month != 'all'):
...
def time_stats(df):
    ...
    df['day_of_week'] = df['Start Time'].dt.dayofweek
    print("Most common day of the week", DAYS[df['day_of_week'].mode()[0]])
